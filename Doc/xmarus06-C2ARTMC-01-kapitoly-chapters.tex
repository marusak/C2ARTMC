...
popísaný z~dvoch hľadísk\,--\,najskôr je stručný prehľad funkcionality nástroja
a~následne je popísaná štruktúra vstupného formátu, ktorý je pre túto prácu
najdôležitejší.
...
regulárnom stromovom model checkingu. \cite{artmc2} V~rámci tejto metódy je konfigurácia pamäte programu
reprezentovaná stromom nad konečnou abecedou. Nekonečná množina konfigurácií je reprezentovaná
stromovými automatmi, ktoré sú zobecnením konečných automatov. Technika následne počíta množinu dosiahnuteľných konfigurácií
reprezenotavaných opäť stromovým automatom.

Nástroj má na vstupe dva súbory, ktoré sa za pomoci dvoch skriptov prevedú na desiatky súborov, ktoré reprezentujú automaty. Z týchto súborov sa následne vytvára analýza. Schéma fungovania nástroja ARTMC je znázornená na obrázku \ref{fig:ARTMC_schema}.
...
\begin{itemize}
    ...
    \item \textit{Ukazateľové položky} - Označené ako \uv{\texttt{next}}. V~programe sú reprezentované prirodzenými číslami začínajúc od 0.
    ...
    \item \textit{Deskriptory 2} - Označené ako \uv{\texttt{gen\_descr}}. Využíva sa iba pri príkaze \texttt{x.next=new} a~označuje automatickú generáciu počiatočného deskriptoru. Nadobúda hodnoty 0 (vypnuté) a~1 (zapnuté). Doporučené nastavenie je 1. Vypnutie sa odporúča iba pokročilým užívateľom so znalosťou práce s~ARTMC.
\end{itemize}
...
\item \texttt{desc\_num} = Počet ukazateľových deskriptorov zvýšený o~1 (viď podsekciu \ref{kap_descr})
...
\item \texttt{restrict\_var} = Automatické generovanie automatu, ktorý garantuje iba jeden výskyt programovej premennej v~konfigurácii. Nadobúda hodnoty 0 (vypnuté) a~1 (zapnuté). Doporučené nastavenie je 1.
\end{itemize}
...
\begin{enumerate}
\item \textit{Deskriptory v~počiatočnej konfigurácii} - Jedná sa o~deskriptory, ktoré sú použité v~súbore \textit{typedefs}. Tieto deskriptory už nemôžu byť znova použité. Číslo najvyššieho použitého deskriptoru je možné získať zo skriptu \texttt{get\_typedef\_descr.t.sh}, ktorý je súčasťou distribúcie ARTMC.
\item \textit{Vlastné deskriptory} - Obvyklý postup je ku každému príkazu typu \texttt{x.next=y} alebo \texttt{x.next=new} priradiť vlastný deskriptor, ktorý ešte nebol použitý. Nemôže byť použitý ani v~konfigurácii\footnote{Iný postup ako tento môže viesť k nesprávnemu fungovaniu nástroja ARTMC a teda sa odporúča len pokročilým užívateľom.}.
\end{enumerate}
...
\newpage
\noindent
Ako príklad uveďme výraz: \codeline{if (x->data -1 == 0)} po úprave výrazu je ho možné prepísať ako: \codeline{if (x->data == 1)}
A~napríklad kód:
...
Tento proces je znázornený na~obrázku \ref{fig:lex}.
...
vykonal.\footnote{Táto práca využíva GitHub a~je dostupná na \texttt{https://github.com/marusak/C2ARTMC}.}
...
Prekladač rovnako preskakuje aj časti príkazov, ak dopredu vie, že nasledujúcu informáciu nepotrebuje. Jedná sa napríklad o~príkaz \texttt{return}. Ak sa za príkazom \texttt{return} nevyskytuje kľúčové slovo \texttt{ERROR}, tak môže prekladač zvyšok príkazu zahodiť. Podobné správanie je implementované aj pri funkciách \texttt{malloc} a~\texttt{random\_alloc}, ktorých argumenty nie sú prekladané.
\newpage
